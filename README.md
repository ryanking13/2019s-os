...
하지만 해당 방식으로도 두 점 사이의 거리가 매우 짧은 경우(넉넉하게 1km 이내) asin을 활용하는 계산 방식이 실패하는 경우(거리가 0으로 계산)가 발생한다. 해당 문제를 해결하기 위해서는 두 점을 2차원 유클리드 공간으로 옮겨서 직선거리를 계산하는 것이 상대적으로 낫다고 판단한다.(해당 문제에 대해서 absorb 할 수 있으면 해보는 것도)
