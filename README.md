...
asin을 이용하는 경우 제곱근 연산(sqrt)가 필요하다. 해당 연산은 이분탐색을 사용하여 빠르고 비교적 정확하게 계산할 수 있었다.
...
소숫점 6째 자리까지의 정확도를 사용하여 거리 계산을 수행하였고 이로 인하여 최대 1km 정도의 오차가 발생한다. 따라서 테스트를 수행할 때에는 __최소 1km 이상 떨어진 두 좌표에서 오차를 고려한 accuracy를 지정__ 할 것이 요구된다.
...
하지만 해당 방식으로도 두 점 사이의 거리가 매우 짧은 경우(넉넉하게 1km 이내) asin을 활용하는 계산 방식이 실패하는 경우(거리가 0으로 계산)가 발생한다. 해당 문제를 해결하기 위해서는 두 점을 2차원 유클리드 공간으로 옮겨서 직선거리를 계산하는 것이 상대적으로 낫다고 판단한다.

__즉, 가까운 좌표(1km 이내)에 대해서 테스트 할 경우, floating-point 오차로 인하여 거리가  따라서 정확한 결과를 얻기 위해서는 두 위치(파일의 위치 and 디바이스 위치) 사이에 충분한 거리를 두고 테스트를 수행할 것이 요구된다.__
